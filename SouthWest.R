...
map <- spplot(nuts1ss, "SW", col.regions = c("white", "seagreen2"), colorkey=FALSE)
...
growthSouthwestservice <- ggplot(data=Southwest, aes(x=services, y=publicchange, group=BUA11CD, label=name))+
  geom_point()+
  labs(x = "Public Service Levels (Per Capita, 2018)", y = "Changes to Public Services (2011-18)", title = "Public Services in South West Towns: Changes and Per Capita Provision")+
...
Southwestjobs <- ggplot(data=Southwest, aes(x=jobdensity, y=housedep, group=BUA11CD, label=name))+
  geom_point()+
  labs(x = "Job Density (2016)", y = "Household Deprivation (%, 2011)", title = "Job Density and Deprivation Rate")+
...
f <- plot_model(p, dot.size = 6, colors = "seagreen2",terms = c("sw [South West Towns]"), ci.lvl = 0, show.values = TRUE, show.p = TRUE, vline.color = "Grey",axis.lim = c(-7,7), axis.title = "Relative Number of Post Boxes", title = "Post Boxes")
ff <- plot_model(n, dot.size = 6, colors = "seagreen2",terms = c("sw [South West Towns]"), ci.lvl = 0, show.values = TRUE, show.p = TRUE, vline.color = "Grey",axis.lim = c(-7,7), axis.title = "Relative Number of Nursery Schools", title = "Nursery Schools")
ffff <- plot_model(h, dot.size = 6, colors = "seagreen2",terms = c("sw [South West Towns]"), ci.lvl = 0, show.values = TRUE, show.p = TRUE, vline.color = "Grey",axis.lim = c(-7,7), axis.title = "Relative Number of Health Services", title = "Health-related Services")
ffffff <- plot_model(b, dot.size = 6, colors = "seagreen2",terms = c("sw [South West Towns]"), ci.lvl = 0, show.values = TRUE, show.p = TRUE, vline.color = "Grey",axis.lim = c(-30,30), axis.title = "Relative Number of Bus Stops", title = "Bus Stops")
...
x<-tab_model(a,b,c,d,e, f, show.ci=0.95,title="Logistic Regression of Public Services availability" ,p.style = c("numeric"), auto.label=FALSE, dv.labels = c("Mental Health Practitioners", "Hospital", "Job Centre", "Train Station","Further Education College",  "Police Station"), pred.labels = c("Intercept",  "Improvement Index", "Distance from City", "Population", "Land Area","Household Deprivation" ,"South West Dummy", "Job Density"))

w<- plot_model(d,ci.lvl = 0 ,type = "pred", colors = c("seagreen2", "seagreen4"), terms = c("growth", "sw"), title = "Predicted Probability of a Town-based Train Station", axis.title = c( "Improvement Index", "Probability of Present Train Station"), legend.title = "Region")
